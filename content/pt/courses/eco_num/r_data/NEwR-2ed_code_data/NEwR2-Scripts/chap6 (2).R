...
# with function triplot.rda
# Scaling 1
dev.new(title = "Partial RDA scaling 1", noRStudioGD = TRUE)
plot(spechem.physio, 
     scaling = 1, 
     display = c("sp", "lc", "cn"), 
...
# Scaling 2
dev.new(title = "Partial RDA scaling 2", noRStudioGD = TRUE)
plot(spechem.physio, 
     display = c("sp", "lc", "cn"), 
...
dev.new(
  title = "Parsimonious RDA scaling 1", 
  width = 16, 
  height = 8, 
  noRStudioGD = TRUE
)
...
dev.new(
  title = "CCA biplot - without species", 
  width = 16, 
  height = 8, 
  noRStudioGD = TRUE
)
...
## Forward selection for cca, rda and capscale using vegan's ordistep()
## CCA-based example
...
plot(mod, select = logabu > 200, scaling = "none")
# plot(mod, select = logabu > 200) # In older versions of R
...
# Plot only the variables that are significantly correlated
# with axes 1 and 2.

# Select the most characteristic variables.
# Function dimdesc.R belongs to FactoMineR.
