...
dev.new(
  title = "Correlogram of substrate density", 
  noRStudioGD = TRUE
)
plot(subs.correlog)
...
dev.new(
  title = "Mantel correlogram of mite data", 
  width = 9, 
  height = 5, 
  noRStudioGD = TRUE
)
...
mite.trend.fit <- 
  scores(mite.trend.rda2, 
         ...
         scaling = 1)
dev.new(
  title = "Mite Trend Surface Analysis", 
  noRStudioGD = TRUE
)
...
dev.new(
  title = "dbMEM variables (transect)", 
  noRStudioGD = TRUE
)
par(mfrow = c(4, 2))
...
mite.rda2.axes <- 
  scores(mite.dbmem.rda2, 
         ...
         scaling = 1)
dev.new(
  title = "dbMEM analysis of mite data", 
  width = 8, 
  height = 6, 
  noRStudioGD = TRUE
)
...
mite.dbmembroad.axes <- 
  scores(mite.dbmem.broad, 
         ...
         scaling = 1)
dev.new(
  title = "dbMEM analysis of mite data - broad scale", 
  noRStudioGD = TRUE
)
par(mfrow = c(1, 2))
...
mite.dbmemmed.axes <- 
  scores(mite.dbmem.med, choices = c(1,2), 
         ...
         scaling = 1)
dev.new(
  title = "dbMEM analysis of mite data - medium scale", 
  noRStudioGD = TRUE
)
par(mfrow = c(1, 2))
...
dev.new(
  title = "One-step dbMEM analysis of mite data", 
  noRStudioGD = TRUE
)
...
dev.new(
  title = "Biplot of dbMEM analysis result - scaling 2", 
  noRStudioGD = TRUE
)
...
dev.new(
  title = "Multivariate variogram, mites", 
  width = 8, 
  height = 4,
  noRStudioGD = TRUE)
plot(
...
dev.new(
  title = "Links, mite MEM champion model",
  noRStudioGD = TRUE
)
...
dev.new(
  title = "MEM analysis of mite data",
  noRStudioGD = TRUE
)
...
dev.new(
  title = "Connectivity matrices - threshold distances", 
  noRStudioGD = TRUE
)
par(mfrow = c(1,2))
...
dev.new(title = "Barplot of MEM eigenvalues", noRStudioGD = TRUE)
...
dev.new(
  title = "MEM eigenvalues vs Moran's I", 
  noRStudioGD = TRUE
)
...
dev.new(title = "AEM variables, lakes")
par(mfrow = c(3,3))
for(i in 1:7) {
sr.value(data.xy, 
         arbor.points[ ,i], 
...
# The row and column numbers were reverse prior to version 0.8
...
dev.new(title = "Site-by-edges, 10 cross-river transect")
...
# If the global analysis repeatedly returns negative
# adjusted R-squard, try with only the 10 first AEM variables:
# AEM.20 <- rda(sp ~ ., as.data.frame(ex.AEM.vec[ ,1:10]))
...
AEM.fwd <- forward.sel(sp, ex.AEM.vec, adjR2thresh = 2*R2a.AEM)
...
AEM.rda.axes <- 
  scores(sp.AEMsign.rda, 
         ...
         scaling = 1)
dev.new(
  title = "AEM analysis of fictitious data, significant RDA axes", 
  noRStudioGD = TRUE
)
...
(miteenvdet.rda.mso <- 
  mso(mitedet.envdet.rda, 
      ...
      perm = 999))
dev.new(
  title = "MSO plot of the detrended mite-environment RDA", 
  noRStudioGD = TRUE
)
